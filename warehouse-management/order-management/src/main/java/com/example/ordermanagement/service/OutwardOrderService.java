    ...
    public void allocateInventoryForOrder(OutwardOrder order) {
        ...
        for (OutwardOrderItem item : items) {
            ...
            try {
                ...
                // Publish inventory move event to Redis (AVAILABLE -> ALLOCATED)
                InventoryEventDto event = InventoryEventDto.moveEvent(
                    item.getProductId(),
                    InventoryState.AVAILABLE,
                    InventoryState.ALLOCATED,
                    pendingQuantity,
                    warehouseId
                );
        ...
        if (allAllocated) {
            ...
            if (fullyAllocated) {
                ...
                // Publish to Redis so WMS can pick up the allocated order
                redisPublisher.publishAllocatedOrder(order);
            }
        }
    }

    @Transactional
    public OutwardOrder cancelOrder(Long orderId) {
        OutwardOrder order = outwardOrderRepository.findById(orderId)
            .orElseThrow(() -> new IllegalArgumentException("Order not found with id: " + orderId));

        // Check if order can be cancelled
        if (order.getStatus() == OutwardOrderStatus.COMPLETE) {
            throw new IllegalStateException("Cannot cancel a completed order");
        }
        if (order.getStatus() == OutwardOrderStatus.CANCELLED) {
            throw new IllegalStateException("Order is already cancelled");
        }

        // If order was allocated, release inventory back to AVAILABLE
        if (order.getStatus() == OutwardOrderStatus.ALLOCATED || order.getStatus() == OutwardOrderStatus.PROCESSING) {
            List<OutwardOrderItem> items = outwardOrderItemRepository.findByOrderId(orderId);
            Long warehouseId = order.getWarehouseId();

            for (OutwardOrderItem item : items) {
                if (item.getAllocatedQuantity() > 0) {
                    // Publish inventory move event to Redis (ALLOCATED -> AVAILABLE)
                    InventoryEventDto event = InventoryEventDto.moveEvent(
                        item.getProductId(),
                        InventoryState.ALLOCATED,
                        InventoryState.AVAILABLE,
        ...
        order.setStatus(OutwardOrderStatus.CANCELLED);
        order = outwardOrderRepository.save(order);

        // Publish cancelled order to Redis so WMS can handle it
        redisPublisher.publishCancelledOrder(order);
